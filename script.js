document.addEventListener("DOMContentLoaded", () => {
  ...
  const centerX = (window.innerWidth - btnRect.width - 250) / 2;
  ...
  const sbtnRect = sbtn.getBoundingClientRect();
  const centerX1 = (window.innerWidth - sbtnRect.width) / 2;
  const centerY1 = (window.innerHeight - sbtnRect.height) / 2;
  sbtn.style.position = "fixed";
  sbtn.style.left = `${centerX1}px`;
  sbtn.style.top = `${centerY1}px`;

  const pushGreenBtn = () => {
    alert('Я рад, что нашёл тебя среди других! 😀')

  }

  sbtn.addEventListener('click', pushGreenBtn);  
});
